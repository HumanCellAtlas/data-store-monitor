...
def get_ssm_lambda_environment():
    parms = ssm_client.get_parameter(
        Name=f"/{os.environ['DSS_PARAMETER_STORE']}/{os.environ['DSS_INFRA_TAG_STAGE']}/environment"
    )['Parameter']['Value']
...
def get_deployed_lambdas():
    ...
    functions = [f"{name}-{os.environ['DSS_INFRA_TAG_STAGE']}" for name in dirs]
    functions.append(f"dss-{os.environ['DSS_INFRA_TAG_STAGE']}")
